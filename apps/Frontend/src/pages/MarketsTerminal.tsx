...
const getStatus = (market: ApiMarket): MarketCard["status"] => {
  if (market.state === "RESOLVED") return "RESOLVED";
        ...
        <main className={`flex-1 flex flex-col min-w-0 ${isDark ? "bg-bg-dark" : "bg-light-bg"} relative overflow-hidden`}>
          ...
          <header className={`h-20 border-b ${isDark ? "border-border-dark bg-bg-dark/80" : "border-border-gray bg-white/80"} backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-4 sm:px-8`}>
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className={`h-9 w-9 border ${isDark ? "border-border-dark bg-card-dark text-text-muted" : "border-border-gray bg-white text-gray-400"} flex items-center justify-center rounded-sm`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-4 relative z-10">
            {loading ? (
              [1, 2, 3].map((item) => (
                <div key={item} className={`${isDark ? "bg-card-dark border-border-dark" : "bg-white border-border-gray"} border rounded-sm p-6 animate-pulse`}>
                  <div className={`h-4 w-24 rounded ${isDark ? "bg-border-dark" : "bg-gray-200"}`}></div>
                  <div className={`h-7 w-2/3 rounded mt-4 ${isDark ? "bg-border-dark" : "bg-gray-200"}`}></div>
                  <div className={`h-4 w-full rounded mt-3 ${isDark ? "bg-border-dark" : "bg-gray-200"}`}></div>
                </div>
              ))
            ) : error ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                <p className="text-sm font-code text-pro-red uppercase tracking-widest">{error}</p>
              </div>
                ...
                >
                  <div className="flex-1 p-4 sm:p-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className={`px-2 py-0.5 text-[9px] font-bold border uppercase tracking-tighter
                        ...
                        ${market.status === "RESOLVED" ? (isDark ? "bg-gray-500/10 text-gray-300 border-gray-500/30" : "bg-gray-100 text-gray-600 border-gray-200") : ""}
                      `}>
                        {market.status}
                      </span>
                    </div>
                  ...
                  <div className={`w-full md:w-[240px] border-t md:border-t-0 md:border-l ${isDark ? "border-border-dark bg-bg-dark/30" : "border-border-gray bg-light-gray/30"} flex flex-col justify-center p-4 sm:p-6`}>
                    ...
                    <div className="flex items-baseline gap-2 mb-4">
                      ...
                      <span className="text-cyber-blue text-xs font-code">{market.traders} traders</span>
                    </div>
                    ...
                    <button className="mt-4 w-full py-2 bg-cyber-blue/10 border border-cyber-blue/30 text-cyber-blue text-[10px] font-bold uppercase tracking-widest rounded-sm hover:bg-cyber-blue hover:text-white transition-all group-hover:shadow-[0_0_15px_rgba(0,209,255,0.3)]">
                      TRADE_NOW
                    </button>
                  </div>
                </div>
          ...
          <footer className={`h-auto sm:h-10 border-t ${isDark ? "border-border-dark bg-bg-dark" : "border-border-gray bg-white"} px-4 sm:px-8 py-2 sm:py-0 flex flex-col sm:flex-row items-center justify-between z-20 gap-2`}>
            ...
            <div className="text-[10px] font-code text-cyber-blue uppercase tracking-wider">[Live_Market_Feed]</div>
          </footer>
        </main>
      </div>
    </div>
