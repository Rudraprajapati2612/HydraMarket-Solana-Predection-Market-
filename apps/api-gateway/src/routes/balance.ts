...
export const balanceRoute = new Elysia({prefix : '/balance'})
        .use(authPlugin())
        ...
        .get('/', async ({ user, query }) => {
            ...
            if(query.asset){
                const balance = await balanceService.getBalance(user.userId,query.asset);
                ...
                const balance = await balanceService.getAllBalances(user.userId);
                return{
                    success : true,
                    data: balance
                }
        ...
        .get('/history', async ({ user, query }) => {
            ...
            const history = await balanceService.getBalanceHistory(user.userId, {
              asset: query.asset,
